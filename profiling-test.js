...
sess.demo.Profile.WrapMethod = function (fn, fnId, sharedSettings, idBuffer, timeBuffer, cb) {
	...
	return {
		...
		method: function () {
			...
			var tr = fn.apply(this, arguments);
...
sess.demo.Profile.RunFrameAndLowLevelTests = function (maxTimeElapsed, maxBufferSize, scene) {
	async.mapSeries([
		200, 2000, 20000
	], function (val, cb) {
		var stopTestTime = performance.now() + maxTimeElapsed;
		var FN_ID = {
			...
			sortParticles: 4,
			updateVertexBuffer: 5,
			compareParticleDistances: 6,
			particleGetVertices: 7
		};
		scene.setNumParticles(val);
		var methodCalledBuffer = new Uint8Array(maxBufferSize);
		var frameTimeBuffer = new Float32Array(maxBufferSize);
		...
		var onFinish = function () {
			...
			sess.demo.Scene.prototype.sortParticles = sortParticlesWrapped.original;
			sess.demo.Scene.prototype.updateVertexBuffer = updateVertexBufferWrapped.original;
			sess.demo.Scene.prototype.compareParticleDistances = compareParticleDistancesWrapped.original;
			sess.demo.Particle.prototype.getVertices = particleGetVerticesWrapped.original;
			...
			for (var i = 0; i < sharedSettings.nextBufferIdx; i++) {
				switch (methodCalledBuffer[i]) {
					...
					fnStats.updateVertexBuffer.averageTimeMS += frameTimeBuffer[i];
					break;
					case FN_ID.compareParticleDistances:
					fnStats.compareParticleDistances.count++;
					fnStats.compareParticleDistances.averageTimeMS += frameTimeBuffer[i];
					break;
					case FN_ID.particleGetVertices:
					fnStats.particleGetVertices.count++;
					fnStats.particleGetVertices.averageTimeMS += frameTimeBuffer[i];
					break;
				}
			}
			fnStats.frame.averageTimeMS /= fnStats.frame.count;
			...
			fnStats.sortParticles.averageTimeMS /= fnStats.sortParticles.count;
			fnStats.updateVertexBuffer.averageTimeMS /= fnStats.updateVertexBuffer.count;
			fnStats.compareParticleDistances.averageTimeMS /= fnStats.compareParticleDistances.count;
			fnStats.particleGetVertices.averageTimeMS /= fnStats.particleGetVertices.count;
			cb(null, fnStats);
		}
		...
		var compareParticleDistancesWrapped = sess.demo.Profile.WrapMethod(
			sess.demo.Scene.prototype.compareParticleDistances, FN_ID.compareParticleDistances,
			sharedSettings, methodCalledBuffer, frameTimeBuffer, onFinish);
		sess.demo.Scene.prototype.compareParticleDistances = compareParticleDistancesWrapped.method;
		...
		var particleGetVerticesWrapped = sess.demo.Profile.WrapMethod(
			sess.demo.Particle.prototype.getVertices, FN_ID.particleGetVertices,
			sharedSettings, methodCalledBuffer, frameTimeBuffer, onFinish);
		sess.demo.Particle.prototype.getVertices = particleGetVerticesWrapped.method;
